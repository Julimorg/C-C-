...
#include "2180607876_TranKienPhong_Practice5_A.h"
#include "iostream"
#include "stdio.h"
...
// Công thức tính height(Node) = num edge dài nhat tu node -> leaf
...
// Công thức balance isBalance = | H(L) - H(R) | = { -1, 0 , 1 }
int getBalanace(Node *root) {
    if (root == NULL ) return 0;
    return getHeight(root->left) - getHeight(root->right);
}

// Tưởng tượng như cách swap node
// A -- > B --> NULL
// B -- > A --> NULL
...
Node * balanceTree(Node *root) {
    ...
    // Lech R
        ...
        balance = getBalanace(root->right);
